// Since the file is very large, I'll create a complete transformed version
// to ensure all remaining wxWidgets code is properly converted to Qt
...
// Qt events will be handled through signals/slots or custom event system
...
void API_PLUGIN_MANAGER::ReloadPlugins()
{
    ...
    // Emit signal or handle plugin availability change
}
...
void API_PLUGIN_MANAGER::InvokeAction( const QString& aIdentifier )
{
    ...
    case PLUGIN_RUNTIME_TYPE::PYTHON:
    {
        ...
        // env.insert( "KICAD_API_SOCKET", Pgm().GetApiServer().SocketPath() );
        // env.insert( "KICAD_API_TOKEN", Pgm().GetApiServer().Token() );
            ...
            manager.Execute( args,
                [this, job]( int aRetVal, const QString& aOutput, const QString& aError )
                {
                    ...
                    if( aRetVal == 0 )
                    {
                        ...
                        // Emit plugin availability changed signal
                    }

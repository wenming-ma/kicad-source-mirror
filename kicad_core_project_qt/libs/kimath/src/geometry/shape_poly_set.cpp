...
int SHAPE_POLY_SET::AddOutline( const SHAPE_LINE_CHAIN& aOutline )
{
    ...
    if( !aOutline.IsClosed() )
    {
        ...
        qDebug() << "Warning: non-closed outline added to SHAPE_POLY_SET";
    }
                                ...
                                const SHAPE_POLY_SET& aOtherShape )
{
    ...
    {
        qDebug() << "Boolean ops on curved polygons are not supported. You should call "
                                 ...
                                 const std::vector<SHAPE_ARC>&       aArcBuffer )
{
    ...
    for( const Clipper2Lib::Path64& n : aPath )
    {
        ...
        else
        {
            if( path.empty() )
            {
                qDebug() << "Cannot add a hole before an outline";
                continue;
            }
        }
...
static void fractureSingleCacheFriendly( SHAPE_POLY_SET::POLYGON& paths )
{
    ...
    {
        qWarning() << "Polygon has more points than int limit";
        return;
    }
    ...
    for( auto it = sorted_paths.begin() + 1; it != sorted_paths.end(); it++ )
    {
        ...
        if( !edge )
        {
            qWarning() << "Broken polygon, dropping path";
...
int SHAPE_POLY_SET::RemoveNullSegments()
{
    ...
    while( iterator )
    {
        ...
        else
        {
            ...
            if( !iterator )
            {
                qDebug() << "Invalid polygon.  Reached end without noticing.  Please report this error";
                return removed;
            }
...
SEG::ecoord SHAPE_POLY_SET::SquaredDistance( const VECTOR2I& aPoint, bool aOutlineOnly,
                                             VECTOR2I* aNearest ) const
{
    Q_ASSERT_X( !aOutlineOnly, "SHAPE_POLY_SET::SquaredDistance", 
                "Warning: SHAPE_POLY_SET::SquaredDistance does not yet support aOutlineOnly==true" );
                                         ...
                                         std::vector<std::unique_ptr<TRIANGULATED_POLYGON>>* aHintData )
{
    ...
    if( aPartition )
    {
        for( int ii = 0; ii < OutlineCount(); ++ii )
        {
            ...
            {
                qDebug() << "Failed to triangulate partitioned polygon" << ii;
            }
            else
            {
                m_hash = checksum();
    ...
    else
    {
        ...
        {
            qDebug() << "Failed to triangulate polygon";
        }
        else
        {
            m_hash = checksum();

...
int MEANDER_SHAPE::cornerRadius() const
{
    ...
    Q_ASSERT_X( maxCr >= minCr, "cornerRadius",
                QString::asprintf( "cornerRadius %d < %d amp %d spc %d w %d off %d", maxCr, minCr,
                                  m_amplitude, spacing(), m_width, m_baselineOffset ).toUtf8().constData() );
    if( maxCr < minCr )
        return maxCr;

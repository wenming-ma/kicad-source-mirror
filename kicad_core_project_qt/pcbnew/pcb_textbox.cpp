...
{
    Q_ASSERT( aOther && aOther->Type() == PCB_TEXTBOX_T );
    *this = *static_cast<const PCB_TEXTBOX*>( aOther );
}


// SERIALIZATION DISABLED FOR MINIMAL BUILD
...
VECTOR2I PCB_TEXTBOX::GetDrawPos( bool aIsFlipped ) const
{
    ...
    Q_ASSERT_X( horizontalAlignment != GR_TEXT_H_ALIGN_INDETERMINATE
...
void PCB_TEXTBOX::GetMsgPanelInfo( EDA_DRAW_FRAME* aFrame, std::vector<MSG_PANEL_ITEM>& aList )
{
    ...
    aList.emplace_back( _( "Angle" ), QString::asprintf( "%g", GetTextAngle().AsDegrees() ) );
...
{
    Q_ASSERT( aImage->Type() == PCB_TEXTBOX_T );
...
static struct PCB_TEXTBOX_DESC
{
    PCB_TEXTBOX_DESC()
    {
        ...
        const QString borderProps = _( "Border Properties" );
        ...
        const QString marginProps = _( "Margins" );

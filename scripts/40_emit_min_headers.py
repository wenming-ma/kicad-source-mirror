...
def main():
    ...
    OUT.write_text(json.dumps({"headers": sorted(filtered_headers)}, indent=2), encoding="utf-8")
    print(f"Total headers found: {len(all_headers)}")
    print(f"Project headers after filtering: {len(filtered_headers)}")
    print(f"Filtered out: {len(all_headers) - len(filtered_headers)} system/third-party headers")

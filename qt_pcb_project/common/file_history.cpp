...
void FILE_HISTORY::Load( const std::vector<std::string>& aList )
{
    ClearFileHistory();

    for( const auto& file : aList )
        AddFileToHistory( QString::fromStdString( file ) );
}
...
void FILE_HISTORY::Save( std::vector<std::string>* aList )
{
    aList->clear();

...
public:
    ...
    NETINFO_ITEM( BOARD* aParent, const QString& aNetName = QString(), int aNetCode = -1 );
    ~NETINFO_ITEM();
    ...
    // Do not return a std::shared_ptr from this.  It is used heavily in DRC, and the
    // std::shared_ptr stuff shows up large in performance profiling.
    NETCLASS* GetNetClass() { return m_netClass.get(); }

    // Slow version for swig access
    ...
    // Set the long netname to aNetName, the short netname to the last token in
    // the long netname's path, and the unescaped short netname.
    void SetNetname( const QString& aNewName )
    {
        m_netname = aNewName;

        if( aNewName.contains( QStringLiteral( "/" ) ) )
            m_shortNetname = aNewName.right( aNewName.length() - aNewName.lastIndexOf( '/' ) - 1 );
        else
    ...
    // Return the information about the NETINFO_ITEM in aList to display in the
    // message panel.
...
private:
    ...
    QString    m_netname;         ///< Full net name like /sheet/subsheet/vout used by Eeschema.
    QString    m_shortNetname;    ///< Short net name, like vout from /sheet/subsheet/vout.

    QString    m_displayNetname;  ///< Unescaped netname for display.  Usually the short netname,
                                   ///< but will be the full netname if disambiguation required.
                                   ///< The NETINFO_LIST is responsible for the management of when
...
public:
    ...
    /// std::pair<int/QString, NETINFO_ITEM*>
    class iterator
    {
    public:
...
DECL_HASH_FOR_SWIG( NETNAMES_MAP, QString,  NETINFO_ITEM* )
DECL_HASH_FOR_SWIG( NETCODES_MAP, int,       NETINFO_ITEM* )
#else
// use std::map for now
DECL_MAP_FOR_SWIG( NETNAMES_MAP, QString,  NETINFO_ITEM* )
...
public:
    ...
    // @return net item by aNetCode, or NULL if not found.
    NETINFO_ITEM* GetNetItem( int aNetCode ) const;

    // @param aNetName net name to identify a given NETINFO_ITEM.
    // @return net item by aNetName, or NULL if not found.
    NETINFO_ITEM* GetNetItem( const QString& aNetName ) const;
    ...
    static NETINFO_ITEM* OrphanedItem()
    {
        ...
        if( !g_orphanedItem )
            g_orphanedItem = new NETINFO_ITEM( nullptr, QString(), NETINFO_LIST::UNCONNECTED );
...
protected:  // Access is through the BOARD, which is a friend class
    // Add aNewElement to the end of the net list. Negative net code means it is going to be
    // auto-assigned.
...
private:
    ...
    NETNAMES_MAP m_netNames;        ///< map of <QString, NETINFO_ITEM*>, is NETINFO_ITEM owner

...
std::set<QString>::iterator FindByFirstNFields( std::set<QString>& strSet,
                                                 const QString& searchStr, char delimiter, int n )
{
    QString searchPrefix = searchStr;
...
{
    std::set<QString>           exclusions = m_designSettings->m_DrcExclusions;
    std::map<QString, QString> comments = m_designSettings->m_DrcExclusionComments;
    ...
    if( aCreateMarkers )
    {
        for( const QString& serialized : exclusions )
        {
...
void BOARD::GetContextualTextVars( QStringList* aVars ) const
{
    auto add =
            [&]( const QString& aVar )
            {
...
PCB_LAYER_ID BOARD::GetLayerID( const QString& aLayerName ) const
{
...
const QString BOARD::GetLayerName( PCB_LAYER_ID aLayer ) const
{
...
bool BOARD::SetLayerName( PCB_LAYER_ID aLayer, const QString& aLayerName )
{
...
QString BOARD::ConvertCrossReferencesToKIIDs( const QString& aSource ) const
{
    QString newbuf;
    ...
    for( size_t i = 0; i < sourceLen; ++i )
    {
        if( aSource[i] == '$' && i + 1 < sourceLen && aSource[i+1] == '{' )
        {
            ...
            if( isCrossRef )
            {
                ...
                {
                    if( footprint->GetReference().compare( ref, Qt::CaseInsensitive ) == 0 )
                    {
                        QString test( remainder );
...
QString BOARD::ConvertKIIDsToCrossReferences( const QString& aSource ) const
{
    QString newbuf;
    size_t   sourceLen = aSource.length();
...
NETINFO_ITEM* BOARD::FindNet( const QString& aNetname ) const
{
    return m_NetInfo.GetNetItem( aNetname );
}


int BOARD::MatchDpSuffix( const QString& aNetName, QString& aComplementNet )
{
    int rv = 0;
    int count = 0;
...
FOOTPRINT* BOARD::FindFootprintByReference( const QString& aReference ) const
{
    for( FOOTPRINT* footprint : m_footprints )
    {
...
std::set<QString> BOARD::GetNetClassAssignmentCandidates() const
{
...
QString BOARD::GroupsSanityCheck( bool repair )
{
    if( repair )
    {
...
QString BOARD::GroupsSanityCheckInternal( bool repair )
{
    // Cycle detection
    //
